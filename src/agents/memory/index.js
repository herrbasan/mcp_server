...
export async function reflect_on_session(args, context) {
    const { gateway } = context;
    const { sessionSummary } = args;
    
    const allMemoriesContext = memories.memories.map(m => 
        `[#${m.id}] [${m.category}] ${m.domain ? `[${m.domain}]` : ''} Conf:${m.confidence || 0.5}\n${m.text}`
    ).join('\n---\n');

    const reflectionPrompt = `Analyze this session summary against existing memories. Find reinforcements, contradictions, or new patterns.
Session:
${sessionSummary}
...
Respond with proposed updates in JSON matching this schema:
{
  "changes": [
    { "action": "create", "text": "...", "category": "proven|anti_patterns|hypotheses", "domain": "optional", "reason": "..." },
    { "action": "update", "id": 123, "text": "...", "category": "...", "reason": "..." },
    { "action": "reinforce", "id": 123, "reason": "..." },
    { "action": "decrease", "id": 123, "reason": "..." }
  ]
}`;

    const response = await gateway.chat({
        model: context.config.models?.synthesis || process.env.DEFAULT_MODEL || 'default',
        messages: [{ role: 'user', content: reflectionPrompt }],
        systemPrompt: "You are a memory analysis assistant. Always return pure JSON."
    });

    return { content: [{ type: 'text', text: response.content }] };
}

export async function apply_reflection_changes(args, context) {
    const { changes } = args;
    const results = [];
    ...
    for (const change of changes) {
        switch (change.action) {
            case 'create':
                const cRes = await remember({ text: change.text, category: change.category, domain: change.domain }, context);
                results.push(`Created: ${cRes.content[0].text}`);
                break;
            case 'update':
                const uRes = await update_memory({ id: change.id, text: change.text, category: change.category, domain: change.domain }, context);
                results.push(`Updated #${change.id}: ${uRes.content?.[0].text || uRes.isError}`);
                break;
            case 'reinforce': {
                const memory = memories.memories.find(m => m.id === change.id);
                if (memory) {
                    memory.confidence = Math.min(1.0, (memory.confidence || 0.5) + 0.2);
                    memory.observations = (memory.observations || 1) + 1;
                    results.push(`Reinforced #${change.id}`);
                }
                break;
            }
            case 'decrease': {
                const memory = memories.memories.find(m => m.id === change.id);
                if (memory) {
                    memory.confidence = Math.max(0.0, (memory.confidence || 0.5) - 0.2);
                    results.push(`Decreased #${change.id}`);
                }
                break;
            }
        }
    ...
    saveMemories();
    return { content: [{ type: 'text', text: `Applied ${changes.length} changes:\n${results.join('\n')}` }] };
}
