...
└────┬────────┬────────┬────────┬─────────────────────────────┘
     ...
     ▼        ▼        ▼        ▼
┌─────────┐ ┌──────┐ ┌──────┐ ┌─────────┐
│LMStudio │ │Ollama│ │Gemini│ │ Copilot │
...
│                  Provider Endpoints                          │
├──────────────┬──────────────┬──────────────┬────────────────┤
│  LM Studio   │   Ollama     │    Gemini    │ GitHub Copilot │
│ (WebSocket)  │ (HTTP REST)  │  (REST API)  │  (REST API)    │
│ localhost:   │ localhost:   │   Google     │   GitHub/      │
...
Router.predict()
    ...
    │
    ├─► Capability check
    │   └─► Error: "Provider 'copilot' does not support embeddings"
    │
    ▼
Adapter.predict()
    │
