...
- **Zero Dependencies:** Default to building it yourself using raw standard libraries. The test is simple: if you understand the problem well enough to judge whether a library is good, you have the knowledge to write it yourself. If you don't understand it that well, you're choosing by false metrics (popularity, age, stars). Only use external dependencies for things genuinely outside your ability to build — complex infrastructure like ffmpeg, or domains where you lack the expertise entirely. Then validate the dependency works for your specific needs before committing to it. Two reasons for the default: (1) **Longevity** — code that runs untouched for 10+ years is only possible when the dependency tree isn't a house of cards. (2) **Understanding** — when you build it, you own the solution. When you import it, you're importing assumptions you may not share. If you find an elegant solution in an existing library, copy the pattern and make it your own — internalize it rather than depending on it.
